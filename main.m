...
for j = 1
    ...
    %NLM
    %t:搜索框半径；f:相似框半径；h:滤波频率百分比,function  [output]=NLmeansfilter(input,t,f,h)
...
%     yfilter2=NLmeansfilter(yg2,5,2,10);
    ...
    fprintf('退化图PSNR=%4.2f\n',Mypsnr(yg,ycimage));
    fprintf('退化图SSIM=%6.4f\n',ssim(yg,ycimage));
    fprintf('PSNR=%4.2f\n',Mypsnr(yg-b,ycimage));
    fprintf('SSIM=%6.4f\n',ssim(yg-b,ycimage));
    %     imwrite(yg,strcat('D:\桌面\实验对比结果\ours\92\',num2str(j),'.bmp'),'bmp');
    %     imwrite(ycimage,strcat('D:\桌面\实验对比结果\ours\92\',num2str(j),'_clear.bmp'),'bmp');
    %     imwrite(clearImg,strcat('D:\桌面\实验对比结果\ours\92\',num2str(j),'_result.bmp'),'bmp');
    %     imwrite(clearImg,strcat('D:\桌面\实验对比结果\real_image\Ours\',num2str(j),'_result.jpg'),'jpg');
    %     imwrite(bias,strcat('D:\桌面\偏置估计实验\chebyshev_bias\k=9\202\',num2str(j),'_bias.bmp'),'bmp');
    %     fprintf('DE=%6.4f\n',DE(yg-b));
    %     fprintf('CV=%6.4f\n',cv(yg-b));
    %
